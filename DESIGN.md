...
```css
--fs-display: 28px;  /* the partial average. Nothing else, on any page. */
...
```
| Course | Code | Cr | Partial | Rank | Final | To pass |
```

- One `<tr>` per course. All courses visible without scrolling.
- `Cr`, `Partial`, `Rank`, `Final` use `.num` (mono, tabular, right-aligned).
...
- The partial average is the `--fs-display` number. **Nothing else on any page
  is.**
